...
export function composeComment(
  ...
  const sections: string[] = [COMMENT_HEADER, "## xray", ""];
  ...
  if (reviewLines > 0 && testLines > 0) {
    summary += ` · **Review ~${reviewLines} lines of logic** (${testLines} lines are tests)`;
  }
  ...
  sections.push(
    "<sub>generated by [xray](https://github.com/kasrakhosravi/xray)</sub>"
  );
